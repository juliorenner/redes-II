...
int main (int argc, char *argv[])
{
  ...
  // ripRouting.SetInterfaceMetric (routerNode_B.Get(0), 3, 10);
  // ripRouting.SetInterfaceMetric (routerNode_C.Get(0), 1, 10);
  ...
  udpechoClientApp_appLink_T_R.Stop (Seconds (130.000));
  ...
  udpechoServerApp_appLink_T_R.Stop (Seconds (130.000));

  // Create the animation object and configure for specified output
  AnimationInterface anim ("topology_2_rip.xml");

  anim.EnablePacketMetadata();
  anim.EnableIpv4RouteTracking ("topology_2_routing_rip.xml", Seconds(1), Seconds(131), Seconds(1));
  ...
  Simulator::Stop (Seconds (131.000));
  Simulator::Run ();
  Simulator::Destroy ();
  NS_LOG_INFO ("Done.");
